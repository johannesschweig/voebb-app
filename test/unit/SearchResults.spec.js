...
describe('SearchResults.vue', () => {
  it('renders search results if any', () => {
    ...
    expect(wrapper.findAll('tr').at(0).find('td').text()).toMatch(/foo \(Buch\).*blub {2}- {2}1234/s)
    ...
    // correct text per row
    expect(wrapper.findAll('tr').at(1).find('td').text()).toMatch(/foo2 \(Buch2\).*blub2 {2}- {2}1235/s)
